import { Directive, ElementRef, forwardRef, Inject, Input, Optional, Renderer2, SkipSelf } from '@angular/core';
...
function initDirectionsGrid(children: NavItem[], navItem: NavItem, gridSize: number): void {
  const getFromDirection = (direction: Direction): NavItem | undefined => {
    ...
    switch (direction) {
      ...
      case 'down':
        return (index + gridSize < children.length) ? children[index + gridSize] : undefined;
      case 'right':
        return (index !== children.length - 1) && ((index + 1) % gridSize !== 0) ? children[index + 1] : undefined;
      case 'left':
        return (index !== 0) && (index % gridSize !== 0) ? children[index - 1] : undefined;
      case 'tab':
        return (index + gridSize < children.length)
          ? children[index + gridSize]
          : ((index !== children.length - 1) && ((index + 1) % gridSize !== 0)) ? children[index + 1] : undefined;
      case 'tabshift':
        return (index - gridSize >= 0)
          ? children[index - gridSize]
          : (index !== 0) && (index % gridSize !== 0) ? children[index - 1] : undefined;
    }
  }
...
function removeDirectionsGrid(children: NavItem[], navItem: NavItem, gridSize: number): void {
...
@Directive({
  selector: '[navGrid]',
  providers: [{provide: NAV_ITEM_TOKEN, useExisting: forwardRef(() => NavGridDirective)}]
})
